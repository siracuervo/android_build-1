...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^lego_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $LEGO_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       LEGO_BUILD=
    fi
    export LEGO_BUILD
...
function setpaths()
{
    ...
    if [ "$GCC_VERSION_ARM" = "4.7" ]; then
            toolchaindir=arm/arm-eabi-sm47/bin
    else
            toolchaindir=arm/arm-eabi-SM/bin
    fi
            if [ -d "$gccprebuiltdir/$toolchaindir" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the LEGO github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var LEGO_VERSION)
        ZIPFILE=lego-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.lego.device=$LEGO_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $LEGO_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/lego/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
